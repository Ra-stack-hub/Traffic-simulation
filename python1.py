...
def start_dashboard(tsys):
    ...
    root.geometry("420x370")  # thoda height badhaya toggle ke liye
    ...
    # --- Weight Sensor Toggle ---
    def toggle_weight():
        tsys.toggle_weight_mode()
        if tsys.weight_mode:
            weight_btn.config(text="Weight Sensor: ON", bg="green")
        else:
            weight_btn.config(text="Weight Sensor: OFF", bg="red")

    weight_btn = tk.Button(
        root, text="Weight Sensor: OFF",
        font=("Arial", 10, "bold"),
        fg="white", bg="red",
        command=toggle_weight
    )
    weight_btn.pack(pady=5)
    ...
    def update_dashboard():
        ...
        # Update densities + weights
        densities = {d: tsys.get_density(d) for d in DIRECTIONS}
        if tsys.weight_mode:
            weights = {d: tsys.get_lane_weight(d) for d in DIRECTIONS}
            density_text.config(
                text=f"N:{densities[NORTH]} ({weights[NORTH]}kg)  "
                     f"E:{densities[EAST]} ({weights[EAST]}kg)  "
                     f"S:{densities[SOUTH]} ({weights[SOUTH]}kg)  "
                     f"W:{densities[WEST]} ({weights[WEST]}kg)"
            )
        else:
            density_text.config(
